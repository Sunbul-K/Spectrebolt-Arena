...
const BANNED_WORDS = ['fuck','mother','father','sister','brother','kids','kys','jerk','terror','muslim','islam','quran','bible','hindu','buddh','christiani','jew','judaism','tower','torah','athei','agnos','god','talmud','vishnu','shiva','sikh','corpse','rotten','jork','kals','kalb','good','bad','laden','obama','biden','bush','boxers','panti','sarm','madaf','dork','like','fathead','dullard','moron','dimwit','nimrod','pimp','nitwit','teez','imbecile','ass','3ars','asshole','douchebag','twat','groom','badass','sex','seg','penis','vagin','molest','anal','kus','sharmoot','khara','ukht','akh','abo','umm','anus','virgin','suck','blow','tit','oral','rim','69','zinji','breast','brest','zib','uterus','dumbass','boob','testi','balls','nut','egg','shit', 'nigg', 'bitch', 'slut', 'nazi', 'hitler', 'milf', 'cunt', 'retard', 'dick', 'diddy', 'diddle', 'epste', 'rape', 'pedo', 'rapis','porn','mussolini','musolini','stalin','trump','cock', 'israel','genocide','homicide','suicide','genocidal','suicidal','homicidal','hog','pussy','twin','9/11','murder','goy','faggot','fagot','piss','negro','bastard','nipp','vulva','sperm','slave','bend','racial','racist','prostitute','prick','orgas','orgie','orgi','orge','mastur','masterb','jackass','horny','handjob','cum','finger','fetish','ejac','devil','demon','crotch','whore','hoe','clit','cocaine','coke','drug','dealer','weed','butt','bang','child','bond','meat','babe','baby','touch','harass','jin','tahar','maniac','manyook','lick','kiss'];
const WORD_ONLY_BANS = ['ass'];

const SAFE_SUBSTRING_BANS = ['boob','kalb','dork','tit','nut','egg','ass','twat','akh','abo','umm','anus','oral','rim','uterus','epstein','rape','goy','nipp','orgas','orgie','orgi','orge','hoe','weed','cum','jin','imbecile','nitwit','dullard','moron','dimwit','nimrod','madaf','biden','obama','laden','kals','kalb','good','bad','bush','boxers','muslim','islam','kys','like','teez','groom','khara','zib','nazi','diddy','ejac','coke','dealer','meat','babe','maniac','manyook','kus','kids'];
...
function isValid(name) {
    if (!/^[a-zA-Z0-9]{1,15}$/i.test(name)) return false;
...
function buildFinalResults() {
  ...
  Object.values(players).forEach(p => {
    ...
    arr.push({ id: p.id, name: p.name ?? 'Player', score, isBot: false });
  });

  Object.values(bots).forEach(b => {
    const score = Number(b.score) || 0;
    arr.push({ id: b.id, name: b.name ?? 'Bot', score, isBot: true });
  });
...
function sanitizeInput(raw = {}) {
  ...
  input.moveX = typeof raw.moveX === 'number' && Number.isFinite(raw.moveX) ? Math.max(-1, Math.min(1, raw.moveX)) : 0;
  input.moveY = typeof raw.moveY === 'number' && Number.isFinite(raw.moveY) ? Math.max(-1, Math.min(1, raw.moveY)) : 0;

  input.angle = typeof raw.angle === 'number' && Number.isFinite(raw.angle) ? raw.angle : 0;

  input.sprint = !!raw.sprint;
  input.shoot = !!raw.shoot;
  input.space = !!raw.space;
  return input;
}
...
io.on('connection', socket => {
    socket.on('joinGame', (data) => {
        ...
        rawName = rawName.replace(/[\x00-\x1F\x7F]/g, '').slice(0, 32);
        let wasRenamedForFilter = false;
        let name = rawName.slice(0,15);
        ...
        if (!autogenerated && containsBannedWord(name)) {
            const choices = ['Imbecile', 'Nitwit', 'Nimrod', 'Dimwit', 'Moron','Dullard','Fathead','Dork'];
            const pick = choices[(Math.random() * choices.length) | 0];
            name = pick + randomDigits();
            console.log(`Blocked name: "${rawName}", renamed to ${name}.`);
            autogenerated = true;
            wasRenamedForFilter = true;
        }
        if (!autogenerated && /(.)\1{2,}/.test(name)) {
            const choices = ['Imbecile', 'Nitwit', 'Nimrod', 'Dimwit', 'Moron','Dullard','Fathead','Dork'];
            const pick = choices[(Math.random() * choices.length) | 0];
            name = pick + randomDigits();
            console.log(`Blocked name: "${rawName}", renamed to ${name}. (Reason: Repeated characters)`);
            autogenerated = true;
            wasRenamedForFilter = true;
        }
        if (!isValid(name)) {
        ...
        handleSuccessfulJoin(socket, name, forcedSpectator, waitingForRematch, wasRenamedForFilter);
    ...
    socket.on('disconnect', () => { 
        ...
        if (p) {
            ...
            if (res && res.isNew) {
                try {
                    io.to(socket.id).emit('personalBestUpdated', { personalBest: res.personalBest, isNew: true });
                } catch (e) {
                }
            }
        }
    ...
    socket.on('rematch', () => {
        ...
        try {
            const res = maybeSavePB(p);
            if (res && res.isNew) {
                try { io.to(socket.id).emit('personalBestUpdated', { personalBest: res.personalBest, isNew: true }); } catch (e) {}
            }
        } catch (e) {
            console.error('Error saving PB on rematch:', e);
        }

        p.wantsRematch = true;
        const rematchers = Object.keys(players).filter(id => players[id] && players[id].wantsRematch);
        ...
        if (matchPhase === 'running') {
            ...
            try { socket.emit('rematchAccepted', { id: p.id, x: p.x, y: p.y, matchTimer, matchPhase, color: p.color }); } catch (e) {}
        ...
        if (matchPhase === 'ended') {
            ...
            if (didReset) {
                for (const id of rematchers) {
                    ...
                    if (!rp) continue;
                    try {
                        io.to(id).emit('rematchAccepted', { id: rp.id, x: rp.x, y: rp.y, matchTimer, matchPhase, color: rp.color });
                    } catch (e) {}
        ...
        try { socket.emit('rematchQueued'); } catch(e) {}
        activeRematches.delete(socket.id);
    });
});
...
setInterval(() => {
  const now = Date.now();
  const cutoffMs = JOIN_CUTOFF_SECONDS * 1000;
  for (const [key, ts] of JOIN_LIMITS.entries()) {
    if (!ts || (now - ts) > cutoffMs) JOIN_LIMITS.delete(key);
  }
}, 60 * 1000);
...
setInterval(() => {
    ...
    Object.values(players).forEach(p => {
        ...
        const input = sanitizeInput(p.input);

        const sprinting = input.sprint && p.stamina > 0;
        let speed = p.isSpectating ? 15 : (sprinting ? SPRINT_SPEED : BASE_SPEED);
    ...
    iterations = Math.min(iterations, MAX_ITERATIONS_PER_TICK);

    for (let it = 0; it < iterations; it++) {
        bulletAccumulator -= STEP;
        Object.values(bullets).forEach(b => {
            if (!b) return;
            if (!Number.isFinite(b.x) || !Number.isFinite(b.y) || !Number.isFinite(b.angle) || !Number.isFinite(b.speed)) {
                delete bullets[b.id];
                return;
            }
        ...
        if (collidesWithWall(b.x, b.y, BULLET_RADIUS) || b.x < 0 || b.x > MAP_SIZE || b.y < 0 || b.y > MAP_SIZE) {
          ...
          return;
            ...
            for (const target of [...livePlayers, ...liveBots]) {
                ...
                let damage = 10;
                if (target.wasRenamedForFilter === true) {
                    damage += 5;
                }
